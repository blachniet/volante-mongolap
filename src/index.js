module.exports = {
  ...
  props: {
    ...
    allowedNamespaces: [],       // IMPORTANT: limit the allowed mongo namespaces, make sure you set this,
                                 //            leaving it empty is a security risk
    routePrefix: '/api/metrics', // the route prefix to use
  ...
  events: {
    'VolanteExpress.router'(router) {
      ...
      /**
       * @openapi
       *  /api/metrics/{namespace}/query:
       *    post:
       *      tags:
       *        - metrics
       *      summary: query metrics stored in mongo
       *      description: performs an aggregated query against the metrics collected in the specified namespace
       *      parameters:
       *        - name: namespace
       *          in: path
       *          description: the full mongo namespace (or volante-mongo alias to query)
       *          required: true
       *          type: string
       *      requestBody:
       *        content:
       ...
       *        - JWT: []
       */
      r.route(`${this.routePrefix}/:namespace/query`)
      ...
      .post((req, res) => {
        ...
        this.$.VolanteMongo.aggregate(req.params.namespace, pipeline).then((docs) => {
          // post-processing
          for (let d of docs) {
      ...
      /**
       * @openapi
       *  /api/metrics/{namespace}/insert:
       *    post:
       *      tags:
       *        - metrics
       *      summary: insert a metric document into mongo
       *      description: inserts a metric document into mongo under the given namespace
       *      parameters:
       *        - name: namespace
       *          in: path
       *          description: the full mongo namespace (or volante-mongo alias to query)
       ...
       *        - JWT: []
       */
      r.route(`${this.routePrefix}/:namespace/insert`)
      .all(this.authenticationProxy)
      .post((req, res) => {
        // check if specified namespace is allowed
        if (this.allowedNamespaces.length > 0 &&
            this.allowedNamespaces.indexOf(req.params.namespace) < 0) {
        ...
        this.$.VolanteMongo.insertOne(req.params.namespace, req.body).then((rslt) => {
          res.send(rslt);
        }).catch((err) => {
          res.status(500).send(err);
        });
  ...
  methods: {
    authenticationProxy(req, res, next) {
      // default to passthrough (no authentication), if user sets 'authModule' and 'authMethod'
